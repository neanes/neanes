...
3. Inside the shortest prefix of a long left-to-right, note-only underscore melisma after which every segment end's quantitative-neume span contains its lyric. The prefix is treated as an atomic run for automatic line breaking. An explicit line or page break still takes precedence.
...
Long underscore melisma lyrics use the same single line-breaking pass. For left-to-right melismas made entirely from adjacent notes, Phase 1 calculates each internal boundary from the same preferred glyph- and measure-bar-aware width used by the Knuth-Plass stream. It prohibits automatic breaks inside the shortest prefix after which every segment end's quantitative-neume span contains the lyric, including its punctuation, and fixes that prefix's internal glue at those widths. The measured group runs to the melisma's end or its first explicit break, whichever comes first. The prohibited breaks run through the one after the last note whose span is still narrower than the lyric, so the atomic run also includes the following note when there is one; negative spacing can narrow the span again after it first contains the lyric, so the first containing note is not enough. If the whole measured group is still narrower than the lyric, its centered outer bounds are therefore known and used for collision layout immediately. Otherwise the protected prefix contains the lyric, so Phase 2 can center it beneath whichever longer melisma segment the chosen break produces without changing the segment's outer bounds. Phase 2 centers a lyric only when the segment's underscore, measured before it is shortened to clear the next lyric, is narrower than `lyricsMelismaCutoffWidth`; an underscore hidden only by that shortening leaves the lyric left-aligned. Phase 2 centers these lyrics before laying out any hyphens or underscores, so those that end at a centered lyric see its final position. It centers a lyric only when that moves it right, keeping it clear of the preceding lyric and any line-start hyphen that Phase 1 positioned against the left-aligned lyric. When a lyric follows the segment on the same line, Phase 2 also requires that Phase 1 reserved `lyricsMinimumSpacing` before it, which Phase 1 does when the measured group ends within `lyricsMelismaCutoffWidth` of the left-aligned lyric. Hyphenated melismas, and Greek melismas when Greek melismata are enabled, retain their existing layout. Melkite RTL underscore melismas and underscore melismas that cross a martyria, tempo, or inline text box remain left-aligned.
...
At a break the post-break glue is skipped at line start while the spacer survives, reserving fixed leading room for the hyphen.
...
- The next note's left projection can be absorbed into the post-break glue, so on the same line we do not always need the full $L_{i+1}$.
...
The preceding melisma line is separately clamped to stop at least `lyricsMinimumSpacing` before the next lyric, so aligning that lyric with the neume edge does not extend the line into the gap.
