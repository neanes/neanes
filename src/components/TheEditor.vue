...
function updateTextBox(
  ...
  if ('content' in newValues || 'contentBottom' in newValues) {
    LayoutService.clearTextBoxContentPreview(toRaw(element));
  }
...
function previewTextBoxContent(
  element: TextBoxElement,
  preview: Partial<Pick<TextBoxElement, 'content' | 'contentBottom'>> | null,
) {
  const rawElement = toRaw(element);

  if (preview == null) {
    LayoutService.clearTextBoxContentPreview(rawElement);
    return;
  }

  LayoutService.setTextBoxContentPreview(rawElement, preview);
  saveDebounced(false);
}
