...
`;
  ...
  applyAttractions(
    ...
    if (
      ...
      this.canApplyAgiaAttraction(node)
    ) {
  ...
  private canApplyAgiaAttraction(node: Readonly<NoteAtomNode>) {
    return (
  ...
  private zoIsNatural(index: number, nodes: AnalysisNode[]): boolean {
    ...
    // A note flagged to ignore attractions is exported natural regardless of
    // the contour (the flattening is gated on this flag before attractions are
    // applied), so it must read as natural here too. Otherwise a neighboring ke
    // that leads up to a zo the user forced natural would wrongly fail to
    // sharpen.
    return (
